...
Params parse_params(const std::vector<std::string>& cmd_args, mirrors::board_size_t board_size) {
    ...
    desc.add_options()
            ("solver-version", po::value(&params.solver_version)->default_value(7))
...
std::vector<int> destroy(const Params& params, OriginGrid<Grid<Mirror>>&ggg, const BeamWidth& bw) {
    ...
    switch (params.solver_version) {
        case 7:
            if (params.board_version == "r3") {
                return mirrors::destroy<mirrors::BS_Restore_i7, mirrors::Board_r3>(params, ggg);
            }
            if (params.board_version == "r5") {
                return mirrors::destroy<mirrors::BS_Restore_i7, mirrors::Board_r5>(params, ggg);
            }
            return mirrors::destroy<mirrors::BS_Restore_i7, mirrors::Board_n_i2>(params, ggg);
        default:
