...
Params parse_params(const std::vector<std::string>& cmd_args, mirrors::board_size_t board_size) {
    ...
    desc.add_options()
            ("solver-version", po::value(&params.solver_version)->default_value(5))
            ...
            ("greedy-ratio", po::value<double>(&params.greedy_ratio)->default_value(0));
    po::variables_map vm;
    ...
    std::cerr << "greedy ratio: " << params.greedy_ratio << '\n';
...
std::vector<int> destroy(const Params& params, OriginGrid<Grid<Mirror>>&ggg) {
    switch (params.score_version) {
        case 4: {
            Solver<mirrors::Score_i4, Board, Board_n_i2_Params> solver(params.beam_width, params.greedy_ratio, ggg, {params.score_param});
...
std::vector<int> FragileMirrors::destroy(const std::vector<std::string>& board) {
    ...
    switch (params.solver_version) {
        case 5:
            return mirrors::destroy<mirrors::BS_Restore_i5, mirrors::Board_n_i2>(params, ggg);
        case 6:
            return mirrors::destroy<mirrors::BS_Restore_i6, mirrors::Board_n_i2>(params, ggg);
        default:
