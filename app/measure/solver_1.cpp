...
#include "mirrors/measure/solver_2.hpp"
...
Params parse_params(const std::vector<std::string>& cmd_args, mirrors::board_size_t board_size) {
    ...
    desc.add_options()
            ("solver-version", po::value(&params.solver_version)->default_value(2))
...
std::vector<int> destroy(const Params& params, Grid<mir_t>& ggg, const BeamWidth& bw) {
    switch (params.score_version) {
        case 1: {
            ...
            auto& duplicates = solver.candidate_duplicates();
            for (auto i = 0; i < duplicates.size(); ++i) {
                auto msg = boost::format("Duplicates: Lev %1%: Width: %2% Count: %3% Candidates: %4%\n")
                        % (i+1) % duplicates[i].beam_width % duplicates[i].count % duplicates[i].candidate_count;
                std::cerr << msg.str();
            }
