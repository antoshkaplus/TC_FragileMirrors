...
// Based on Solver_3. Use `derivs_order` for sorting derivatives to save swapping objects.
...
public:
    explicit Solver_4(size_t beam_width_init,
    ...
    std::optional<std::vector<Position>> AddDerivativesNextBoards() {
        auto derivs_order_cmp = [&](const deriv_count_t i_1, const deriv_count_t i_2) {
        ...
        std::nth_element(start,
                         ...
                         derivs_order.end(),
                         derivs_order_cmp);
        std::sort(start, end, derivs_order_cmp);
