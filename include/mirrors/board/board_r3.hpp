...
// Based on Board_r2. Shrink casts with empty lines.
class Board_r3 {
public:
...
public:
    Board_r3(const board_size_t size) :
            casts(4 * size),
            cells(size*size) {
...
private:
    ...
    void TryDestroyCasts(cell_count_t destroyed_cell_idx) {
        ...
        if (cells[loc].neighbours[kDirTop] >= kBorderIndentIdx &&
                cells[loc].neighbours[kDirBottom] >= kBorderIndentIdx) {
            restore.push_back(cells[loc].neighbours[kDirTop] - kBorderIndentIdx);
            restore.push_back(cells[loc].neighbours[kDirBottom] - kBorderIndentIdx);
        }
        if (cells[loc].neighbours[kDirLeft] >= kBorderIndentIdx &&
                cells[loc].neighbours[kDirRight] >= kBorderIndentIdx) {
            restore.push_back(cells[loc].neighbours[kDirLeft] - kBorderIndentIdx);
            restore.push_back(cells[loc].neighbours[kDirRight] - kBorderIndentIdx);
        }
        std::sort(restore.begin(), restore.end(), std::greater());
        for (auto cast_loc : restore) {
