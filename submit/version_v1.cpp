...
vector<int> FragileMirrors::destroy(const vector<string>& board)  {
    version_v1::Board_v6 b(board);
    version_v1::BeamSearch_v2<version_v1::Board_v6> bs;
    b = bs.Destroy(b,
                   500 * pow(100. / b.size(), 2),
                   version_v1::EMPTY_LINES_PARAM[b.size()-50],
                   version_v1::OVERHEAD_PARAM[b.size()-50]);
    auto ps = version_v1::CastNode::ToHistory(b.CastHistory());
    vector<int> res;
    for (auto& p : ps) {
        res.push_back(p.row);
        res.push_back(p.col);
